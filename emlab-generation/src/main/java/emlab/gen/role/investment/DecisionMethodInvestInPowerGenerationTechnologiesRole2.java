...
@Configurable
@NodeEntity
public class DecisionMethodInvestInPowerGenerationTechnologiesRole2<T extends EnergyProducer> extends GenericInvestmentRole<T>
implements
Role<T>,
NodeBacked {
    ...
    public void act(T agent) {
        ...
        for (PowerGeneratingTechnology technology : reps.genericRepository.findAll(PowerGeneratingTechnology.class)) {
            ...
            long futureTimePoint = technology.getExpectedLeadtime() + technology.getExpectedPermittime();
            ...
            if ((expectedInstalledCapacityOfTechnologyInNode + plant.getActualNominalCapacity()) > pgtNodeLimit) {
                ...
                // marketInformationArray.get(1).maxExpectedLoad) {
        ...
        for (PowerGeneratingTechnology technology : eligibleTechnologies) {
            ...
            long futureTimePoint = technology.getExpectedLeadtime() + technology.getExpectedPermittime()
                    + getCurrentTick();
            // logger.warn(agent + " is looking at timepoint " +
            // futureTimePoint);
            ...
            logger.warn("CO2 Price " + expectedCO2Price.toString());

            // logger.warn(expectedCO2Price.toString());
            List<Map<ElectricitySpotMarket, Double>> expectedDemandList = new ArrayList<Map<ElectricitySpotMarket, Double>>(
                    numberOfScenarios);
            // Demand
            Map<ElectricitySpotMarket, Double> tempExpectedDemand = new HashMap<ElectricitySpotMarket, Double>();
            Map<ElectricitySpotMarket, Double> tempUpperExpectedDemand = new HashMap<ElectricitySpotMarket, Double>();
            Map<ElectricitySpotMarket, Double> tempLowerExpectedDemand = new HashMap<ElectricitySpotMarket, Double>();
            ...
            for (ElectricitySpotMarket elm : reps.template.findAll(ElectricitySpotMarket.class)) {
                ...
                for (long time = getCurrentTick(); time > getCurrentTick()
                        - agent.getNumberOfYearsBacklookingForForecasting()
                        && time >= 0; time = time - 1) {
                    gtr.addData(time - getCurrentTick(), elm.getDemandGrowthTrend().getValue(time));
                }
                tempExpectedDemand.put(elm, gtr.predict(futureTimePoint - getCurrentTick()));
                try {
                    tempLowerExpectedDemand.put(elm, gtr.predictLowerBound(futureTimePoint - getCurrentTick(), 0.05));
                } catch (MathException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    logger.warn("Couldn't compute confidence intervals!");
                    tempLowerExpectedDemand.put(elm, gtr.predict(futureTimePoint));
                }
                try {
                    tempUpperExpectedDemand.put(elm, gtr.predictUpperBound(futureTimePoint - getCurrentTick(), 0.05));
                    // logger.warn(Double.toString(gtr.getIntercept()));
                    // logger.warn(Double.toString(gtr.getSlope()));
                    // logger.warn(Double.toString(gtr.getSlopeConfidenceInterval(0.05)));
                } catch (MathException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    logger.warn("Couldn't compute confidence intervals!");
                    tempUpperExpectedDemand.put(elm, gtr.predict(futureTimePoint));
                }
            }
            expectedDemandList.add(0, tempExpectedDemand);
            expectedDemandList.add(1, tempUpperExpectedDemand);
            expectedDemandList.add(2, tempLowerExpectedDemand);
            logger.warn("Expected demand: " + tempExpectedDemand.toString());
            logger.warn("Lower scenario: " + tempLowerExpectedDemand.toString());
            logger.warn("Upper scenario: " + tempUpperExpectedDemand.toString());

            List<MarketInformation> marketInformationArray = new ArrayList<MarketInformation>(numberOfScenarios);
            for (int i = 0; i < numberOfScenarios; i++) {
                marketInformationArray.add(new MarketInformation(market, expectedDemandList.get(i), expectedFuelPrices,
            ...
            for (int i = 0; i < numberOfScenarios; i++) {
                ...
                for (SegmentLoad segmentLoad : market.getLoadDurationCurve()) {
                    ...
                    logger.warn("Electricity price: " + Double.toString(expectedElectricityPrice));
                ...
                logger.warn("Technology ", technology, "has " + runningHours + " running hours, an expected income of "
                        + expectedGrossProfit);
