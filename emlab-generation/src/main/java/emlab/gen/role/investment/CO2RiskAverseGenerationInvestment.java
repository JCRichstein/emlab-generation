...
@Configurable
@NodeEntity
public class CO2RiskAverseGenerationInvestment<T extends EnergyProducer> extends GenericInvestmentRole<T>
implements
Role<T>,
NodeBacked {
    ...
    public void act(T agent) {
        ...
        List<Map<ElectricitySpotMarket, Double>> expectedCO2PriceList = null;
        try {
            expectedCO2PriceList = determineExpectedCO2PriceScenariosInclTax(
                    futureTimePoint, agent.getNumberOfYearsBacklookingForForecasting(), 0, 0.05);
        ...
        List<MarketInformation> marketInformationArray =new ArrayList<MarketInformation>(numberOfScenarios);
        for (int i = 0; i < numberOfScenarios; i++) {
            marketInformationArray.add(new MarketInformation(market, expectedDemand, expectedFuelPrices,
                    expectedCO2PriceList.get(i).get(market).doubleValue(), futureTimePoint));
        }

        // Create double matrix, rows are technologies, columns scenario
        int numberOfTechnologies = (int) reps.powerGeneratingTechnologyRepository.count();
        DenseDoubleMatrix2D payoffMatrix = new DenseDoubleMatrix2D(numberOfTechnologies + 1, numberOfScenarios);
        payoffMatrix.viewRow(0).assign(0); // first row is no investment

        Map<PowerGeneratingTechnology, Integer> technologyToRowMap = new HashMap<PowerGeneratingTechnology, Integer>();
        PowerGeneratingTechnology[] rowToTechnologyArray = new PowerGeneratingTechnology[numberOfTechnologies + 1];
        rowToTechnologyArray[0] = null;

        Map<PowerGeneratingTechnology, double[]> scenarioMap = new HashMap<PowerGeneratingTechnology, double[]>();
        int techIndex = 1;
        for (PowerGeneratingTechnology technology : reps.genericRepository.findAll(PowerGeneratingTechnology.class)) {
            scenarioMap.put(technology, new double[numberOfScenarios]);
            rowToTechnologyArray[techIndex] = technology;
            technologyToRowMap.put(technology, new Integer(techIndex));
            techIndex++;

        }

        // Determining eligible technologies
        Set<PowerGeneratingTechnology> eligibleTechnologies = new HashSet<PowerGeneratingTechnology>();
        for (PowerGeneratingTechnology technology : reps.genericRepository.findAll(PowerGeneratingTechnology.class)) {
            ...
            if ((expectedInstalledCapacityOfTechnology + plant.getActualNominalCapacity())
                    / (marketInformationArray.get(1).maxExpectedLoad + plant.getActualNominalCapacity()) > technology
                    .getMaximumInstalledCapacityFractionInCountry()) { //For the upper scenario
                ...
                // technology);
            } else if (capacityInPipelineInMarket > 0.2 * marketInformationArray.get(1).maxExpectedLoad) {
        ...
        for (int i = 0; i < numberOfScenarios; i++) {
            for (PowerGeneratingTechnology technology : eligibleTechnologies) {
                ...
                for (SegmentLoad segmentLoad : market.getLoadDurationCurve()) {
                    double expectedElectricityPrice = marketInformationArray.get(i).expectedElectricityPricesPerSegment
                ...
                double operatingProfit = - fixedOMCost;

                if (runningHours > plant.getTechnology().getMinimumRunningHours()) {
                    operatingProfit = operatingProfit + expectedGrossProfit;
                }
                ...
                scenarioMap.get(technology)[i] = projectValue / plant.getActualNominalCapacity();
                payoffMatrix.set(technologyToRowMap.get(technology).intValue(), i,
        ...
        // for (Entry<PowerGeneratingTechnology, double[]> entry :
        // scenarioMap.entrySet()) {
        // logger.warn(entry.getKey().toString() + ": " + entry.getValue()[0] +
        // "," + ", " + entry.getValue()[1]
        // + ", "
        // + entry.getValue()[2]);
        // }
        logger.warn(payoffMatrix.toString());

        DenseDoubleMatrix1D bestValueInScenario = new DenseDoubleMatrix1D(payoffMatrix.columns());
        for (int column = 0; column < payoffMatrix.columns(); column++) {
            bestValueInScenario.set(column, payoffMatrix.viewColumn(column)
                    .aggregate(Functions.max, Functions.identity));
        }

        DenseDoubleMatrix2D regretMatrix = (DenseDoubleMatrix2D) payoffMatrix.copy();
        for (int row = 0; row < regretMatrix.rows(); row++) {
            regretMatrix.viewRow(row).assign(bestValueInScenario, Functions.minus);
        }

        DenseDoubleMatrix1D maximumRegret = new DenseDoubleMatrix1D(payoffMatrix.rows());
        for (int row = 0; row < regretMatrix.rows(); row++) {
            maximumRegret.set(row, regretMatrix.viewRow(row).aggregate(Functions.min, Functions.identity));
        }

        bestTechnology = rowToTechnologyArray[getIndexOfMaxValue(maximumRegret)];


        Map<PowerGeneratingTechnology, Double> pessimisticMap = new HashMap<PowerGeneratingTechnology, Double>();
        for (PowerGeneratingTechnology technology : scenarioMap.keySet()) {
            pessimisticMap.put(technology, getMinValue(scenarioMap.get(technology)));
        }


        bestTechnology = getKeyOfMaxValue(pessimisticMap);


        if (bestTechnology != null && pessimisticMap.get(bestTechnology).doubleValue() > 0) {
    ...
    public static int getIndexOfMinValue(DoubleMatrix1D matrix) {
        double minValue = matrix.get(0);
        int index = 0;
        for (int i = 1; i < matrix.size(); i++) {
            if (matrix.get(i) < minValue) {
                minValue = matrix.get(i);
                index = i;
            }
        }
        return index;
    ...
    public static int getIndexOfMaxValue(DoubleMatrix1D matrix) {
        double maxValue = matrix.get(0);
        int index = 0;
        for (int i = 1; i < matrix.size(); i++) {
            if (matrix.get(i) > maxValue) {
                maxValue = matrix.get(i);
                index = i;
            }
        }
        return index;
    ...
    public static PowerGeneratingTechnology getKeyOfMaxValue(Map<PowerGeneratingTechnology, Double> map) {
        Double maxValue = Double.MIN_VALUE;
        PowerGeneratingTechnology maxTechnology = null;
        for (Entry<PowerGeneratingTechnology, Double> current : map.entrySet()) {
            if (current.getValue().doubleValue() > maxValue) {
                maxValue = current.getValue();
                maxTechnology = current.getKey();
            }
        }
    ...
    protected List<Map<ElectricitySpotMarket, Double>> determineExpectedCO2PriceScenariosInclTax(long futureTimePoint,
            long yearsLookingBackForRegression, int adjustmentForDetermineFuelMix, double confidenceLevel)
                    throws MathException {
        List<Map<ElectricitySpotMarket, Double>> expectedCO2PriceScenarios = new ArrayList<Map<ElectricitySpotMarket, Double>>(
                3);
        HashMap<ElectricitySpotMarket, Double> co2PricesCentral = new HashMap<ElectricitySpotMarket, Double>();
        HashMap<ElectricitySpotMarket, Double> co2PricesLow = new HashMap<ElectricitySpotMarket, Double>();
        HashMap<ElectricitySpotMarket, Double> co2PricesHigh = new HashMap<ElectricitySpotMarket, Double>();
        ...
        int i = 0;
        for (ClearingPoint clearingPoint : cps) {
            sr.addData(clearingPoint.getTime()-getCurrentTick(), clearingPoint.getPrice());
            lastPrice = clearingPoint.getPrice();
            averagePrice += lastPrice;
            i++;
        }
        averagePrice = averagePrice / i;
        double expectedCO2PriceCentral;
        double expectedCO2PriceLow;
        double expectedCO2PriceHigh;
        if(i>2){
            expectedCO2PriceCentral = sr.predict(futureTimePoint-getCurrentTick());
            expectedCO2PriceHigh = sr.getIntercept() + (futureTimePoint-getCurrentTick()) * (sr.getSlope() + sr.getSlopeConfidenceInterval(confidenceLevel));
            expectedCO2PriceLow = sr.getIntercept() + (futureTimePoint-getCurrentTick()) * (sr.getSlope() - sr.getSlopeConfidenceInterval(confidenceLevel));
            logger.warn("Central price: " + Double.toString(expectedCO2PriceCentral));
            logger.warn("High price:" + Double.toString(expectedCO2PriceHigh));
            logger.warn("Low price:" + Double.toString(expectedCO2PriceLow));
            expectedCO2PriceCentral = Math.max(0, expectedCO2PriceCentral);
            expectedCO2PriceCentral = Math.min(expectedCO2PriceCentral, government.getCo2Penalty(futureTimePoint));
            expectedCO2PriceHigh = Math.max(0, expectedCO2PriceHigh);
            expectedCO2PriceHigh = Math.min(expectedCO2PriceHigh, government.getCo2Penalty(futureTimePoint));
            expectedCO2PriceLow = Math.max(0, expectedCO2PriceLow);
            expectedCO2PriceLow = Math.min(expectedCO2PriceLow, government.getCo2Penalty(futureTimePoint));
        }else{
            expectedCO2PriceCentral = lastPrice;
            expectedCO2PriceLow = lastPrice;
            expectedCO2PriceHigh = lastPrice;
        }
        // Calculate average of regression and past average:
        expectedCO2PriceCentral = (expectedCO2PriceCentral + averagePrice) / 2;
        for (ElectricitySpotMarket esm : reps.marketRepository.findAllElectricitySpotMarkets()) {
            ...
            double co2PriceCentralInCountry = 0d;
            if (expectedCO2PriceCentral > nationalCo2MinPriceinFutureTick) {
                co2PriceCentralInCountry = expectedCO2PriceCentral;
            } else {
                co2PriceCentralInCountry = nationalCo2MinPriceinFutureTick;
            }
            co2PriceCentralInCountry += reps.genericRepository.findFirst(Government.class).getCO2Tax(futureTimePoint);
            co2PricesCentral.put(esm, Double.valueOf(co2PriceCentralInCountry));

            double co2PriceHighInCountry = 0d;
            if (expectedCO2PriceHigh > nationalCo2MinPriceinFutureTick) {
                co2PriceHighInCountry = expectedCO2PriceHigh;
            } else {
                co2PriceHighInCountry = nationalCo2MinPriceinFutureTick;
            }
            co2PriceHighInCountry += reps.genericRepository.findFirst(Government.class).getCO2Tax(futureTimePoint);
            co2PricesHigh.put(esm, Double.valueOf(co2PriceHighInCountry));

            double co2PriceLowInCountry = 0d;
            if (expectedCO2PriceLow > nationalCo2MinPriceinFutureTick) {
                co2PriceLowInCountry = expectedCO2PriceLow;
            } else {
                co2PriceLowInCountry = nationalCo2MinPriceinFutureTick;
            }
            co2PriceLowInCountry += reps.genericRepository.findFirst(Government.class).getCO2Tax(futureTimePoint);
            co2PricesLow.put(esm, Double.valueOf(co2PriceLowInCountry));
        ...
        logger.warn("High price:" + Double.toString(expectedCO2PriceHigh));
        logger.warn("Low price:" + Double.toString(expectedCO2PriceLow));
        expectedCO2PriceScenarios.add(0, co2PricesCentral);
        expectedCO2PriceScenarios.add(1, co2PricesHigh);
        expectedCO2PriceScenarios.add(2, co2PricesLow);

        logger.warn("Central price: " + co2PricesCentral.toString());
        logger.warn("High price:" + co2PricesHigh.toString());
        logger.warn("Low price:" + co2PricesLow.toString());
        return expectedCO2PriceScenarios;
    }
